...
=== 1: Importance of Private Automation Hub?
...
. Launch the Private Automation Hub web interface and login using the credentials from the RHDP login page.
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
export AAP_URL='<Provide_aap_controller_web_url_from_rhdp_login_page>'
----
...
export ANSIBLE_GALAXY_SERVER_PUBLISHED_URL=$AAP_URL/pulp_ansible/galaxy/published/
export ANSIBLE_GALAXY_SERVER_PUBLISHED_TOKEN=$PAH_API_TOKEN

# Configure the 'certified' repository
export ANSIBLE_GALAXY_SERVER_CERTIFIED_URL=$AAP_URL/pulp_ansible/galaxy/rh-certified/
export ANSIBLE_GALAXY_SERVER_CERTIFIED_TOKEN=$PAH_API_TOKEN

# Configure the 'validated' repository
export ANSIBLE_GALAXY_SERVER_VALIDATED_URL=$AAP_URL/pulp_ansible/galaxy/validated/
export ANSIBLE_GALAXY_SERVER_VALIDATED_TOKEN=$PAH_API_TOKEN

# Configure the 'community' repository
export ANSIBLE_GALAXY_SERVER_COMMUNITY_URL=$AAP_URL/pulp_ansible/galaxy/community/
...
ansible-galaxy collection publish -s $AAP_URL/api/galaxy/ ansible_bootcamp-my_collection-1.0.0.tar.gz --token $PAH_API_TOKEN
----
...
. Launch the Private Automation Hub web interface and login using the credentials from the RHDP login information.
...
+
. Enter the username and password using the credentials provided from the from the RHDP login page and click the **Sign In** button
+ 
...
additional_build_steps:
  prepend_galaxy:
    ...
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_URL='$AAP_URL/pulp_ansible/galaxy/rh-certified/'
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_URL='$AAP_URL/pulp_ansible/galaxy/validated/'
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_URL='$AAP_URL/pulp_ansible/galaxy/community/'
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_PUBLISHED_URL='$AAP_URL/pulp_ansible/galaxy/published/'
...
ansible-galaxy collection build
ansible-galaxy collection publish -s $AAP_URL/api/galaxy/ ansible_bootcamp-my_collection-1.0.1.tar.gz --token $PAH_API_TOKEN
----
