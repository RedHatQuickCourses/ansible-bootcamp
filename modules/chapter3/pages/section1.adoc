...
First, set a new variable for your PAH API Token and AAP URL: 
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
export AAP='<Provide_aap_controller_hostname_rhdp_login_page>'
----

NOTE: Replace `YOUR_API_TOKEN_HERE` With the personal API token you just copied from PAH and <Provide_aap_controller_web_url_from_rhdp_login_page> from the RHDP login page.
...
git remote add origin {replace_gitea_console_url}/{reaplace_gitea_user}/ansible_bootcamp_my_collection.git
...
images:
  base_image:
    name: $AAP/ansible-automation-platform-25/ee-minimal-rhel9:latest
...
podman tag localhost/my-pah-ee:1.0 $AAP/my-pah-ee:1.0
podman push $AAP/my-pah-ee:1.0
----
...
podman tag localhost/my-pah-ee:1.1 $AAP/my-pah-ee:1.1
podman push $AAP/my-pah-ee:1.1
----
