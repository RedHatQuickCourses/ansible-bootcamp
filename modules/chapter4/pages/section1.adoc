...
NOTE: Lab setup should have been previously completed in Managing Ansible Content with Private Automation Hub
...
Create another Gitea repository, separate from the _ansible_bootcamp_my_collection_ repository that was created previously in Managing Ansible Content with Private Automation Hub, which will serve as a working CI/CD pipeline development repository.

. Navigate to your Gitea instance and click the **Sign In** button on the upper right hand corner. Enter the username and password using the credentials provided from the Environment Details page and click the **Sign In** button
...
. Clone the _ansible_bootcamp_ci_cd_pipeline_ repository you just created into your workspace use RHDP login page for the values for below command: 
+
...
. First, set your Private Automation Hub API token and AAP URL as an environment variable
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
